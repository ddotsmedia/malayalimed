...
import { submitPriorAuth, priorAuthList } from '@/lib/insurance';
...
export async function GET() {
  ...
  return NextResponse.json({ data: s.doctorId ? await priorAuthList(s.doctorId) : [], errors: null });
}
export async function POST(request) {
  const s = await requireDoctor();
  if (!s || !s.doctorId) return NextResponse.json({ errors: ['forbidden'] }, { status: 403 });
  const v = parse(S.priorAuth, await request.json().catch(() => ({})));
  if (!v.ok) return NextResponse.json({ errors: [v.error] }, { status: 400 });
  return NextResponse.json({ data: await submitPriorAuth(s.doctorId, v.data), errors: null }, { status: 201 });
}
